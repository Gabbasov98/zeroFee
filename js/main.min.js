...
filterSliders.forEach(slider => {
    ...
    if (rangeSlider) {
        rangeSlider.noUiSlider.on('update', function(values, handle) {
            rangeInputs[handle].innerHTML = Math.round(values[handle])

            const value = Number(values[handle]);
            const percent = ((value - min) / (max - min));
            let containerWidth = $(".filter-slider__float").width()
            let transformSize = containerWidth * percent

            console.log(percent.toFixed(2));
            $(".filter-slider__float-inner").css("transform",`translateX(${transformSize}px)`)
        })
    }
})
